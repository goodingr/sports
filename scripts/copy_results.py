...
# Paths
def copy_results(base_dir: Path = Path("data/forward_test")):
    ensemble_master = base_dir / "ensemble" / "predictions_master.parquet"
    model_types = ["random_forest", "gradient_boosting"]
    
    # Load ensemble predictions
    print(f"Loading ensemble predictions from {ensemble_master}")
    if not ensemble_master.exists():
        print(f"ERROR: Ensemble predictions not found at {ensemble_master}")
        return
