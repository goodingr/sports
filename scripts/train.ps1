...
<#
.SYNOPSIS
    Runs the heavy training pipeline: Ingestion (Results/Injuries) -> Advanced Stats -> Dataset Build -> Model Training.
    This script is intended to be run less frequently (e.g., daily).
#>
...
$logFile = "logs/train_pipeline_{0}.log" -f (Get-Date -Format 'yyyyMMdd_HHmmss')
...
Write-Log "Step 1: Ingesting historical results and injuries..."
try {
    & poetry run python -c "
import datetime
import json
