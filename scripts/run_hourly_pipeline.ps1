...
try {
    ...
    if (-not $datasetReady -or -not $datasetReady.Count) {
        ...
        foreach ($league in $datasetReady) {
            ...
            Write-Log "Training $league model..."
            & poetry run python -m src.models.train --league $league --seasons $leagueSeasons
            if ($LASTEXITCODE -ne 0) {
                Write-Log "WARNING: Training failed for $league"
            } else {
                $trainedLeagues += $league
            }
        }
        if (-not $trainedLeagues.Count) {
            Write-Log "WARNING: No leagues trained successfully"
        }
    }
...
try {
    if (-not $trainedLeagues -or -not $trainedLeagues.Count) {
        Write-Log "WARNING: Skipping prediction step because no trained leagues are available"
    } else {
