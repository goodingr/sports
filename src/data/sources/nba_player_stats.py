"""Pull NBA player game logs via the official nba_api client."""
...
def ingest(
    *,
    ...
    """Download player game logs for the given seasons."""

    season_list = _normalize_seasons(seasons)
    definition = SourceDefinition(
        key="nba_player_stats",
        name="NBA Player Stats API",
        league="NBA",
        category="player_logs",
        url="https://stats.nba.com/",
        default_frequency="daily",
        storage_subdir="nba/player_stats",
    )
    ...
    with source_run(definition) as run:
        ...
        for season in season_list:
            ...
            LOGGER.info("Requesting nba_api Player LeagueGameLog for %s (%s)", season, season_type)
            # player_or_team_abbreviation='P' fetches player stats
            ...
            parquet_path = run.make_path(f"player_gamelog_{season}.parquet")
        ...
        if total_rows:
            ...
            run.set_message(f"Captured {total_rows} nba_api player game log rows")
