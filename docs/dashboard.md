...
```bash
./run_dashboard --host 0.0.0.0 --port 8050 --debug
```

For PowerShell users, the same helper is available as `.\run_dashboard.ps1 --host 0.0.0.0 --port 8050 -DashDebug -- --extra-flags` (pass any extra Dash options after `--`).

`./run_dashboard` and `.\run_dashboard.ps1` are thin wrappers around `poetry run python -m src.dashboard`, so you can pass any Dash CLI flags via `--`.

## Winner Predictions Page (/predictions)

- Navigate to http://localhost:8050/predictions to compare our model's projected winners against sportsbook consensus.
- Filters: league dropdown (same options as the main dashboard) plus a commence-date range picker.
- Summary cards highlight agreement rate, scenarios where we beat the books (and vice versa), mutual wins/losses, and pending games.
- A detailed table lists each matchup with our pick + probability, the sportsbook pick + implied probability, agreement flag, actual result, and our probability edge (in percentage points) for the chosen side.

Use this view to quickly audit disagreements and identify profitable angles where the model historically outperforms the market.
